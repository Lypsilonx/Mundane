...
<!-- Using "Markdown All in One" VSCode extension to render the Table of Contents-->
- [Vision](#vision)
...
The Players will each need to [build a character](#building-a-character).\
The game master needs to [set up a campaign](#setting-up-a-campaingn).\
The game master should have read and understood all the rules before starting to play.

<hr>

## Building a Character

Characters in Mundane have an [importance](#importance), two [classes](#classes), [specialisations](#specializations) and 20 [moods](#moods).\
The following chapters will teach you how to build a character step by step.

First, you will need an idea for a character. Depending on how your GM wants to play, they might have already told you a genre, movie, series or book, your character will be part of in the game.\
When you have thought of a character, take out a piece of paper and write their name on it. This is your **character sheet**.

<hr>

### Importance

This mostly determines what happens when your character [dies](#character-lifespan), but also how they are seen by other characters and [NPC's](#npcs) in-game.\
Choose one of the following and write it down on your character sheet.
...
Every character must have **two classes**.\
You can also build a charter of **pure** class that gains every bonus twice[^1].

[^1]: +1 → +2; Once per session → Twice per session; Choose a → choose two

Write down two of the following classes on your character sheet. You can ignore the Moods column for now.
...
| Acrobat        | +1 on any check that relies on their physique.<hr>+2 on any check that is [impossible](#ability-checks). | IEEP |
...
Every character can have any number of specialisations [^4] from the list below, plus any the GM may add themselves.

[^4]: Though it is not advised to pick too many, as it might get difficult to keep track of all your characters kinks.

Write down any specializations, that you want for your character on your character sheet. The sum of specialization points (SP) will have to add up to 0 (or lower) at the end.

#### Boni
| Specialization    | Description | SP |
| :---------------- | :---------- | -: |
| Neat              | <hr>        |    |
| Bookworm          | Your character can always exclaim "I've read about this...!" and on a successfull [ability check](#ability-checks) the GM will have to tell you something useful about the thing they've "read about". | +1 |
| Happy pills       | Once a session you can set your characters [mood](#moods) to any mood you want. | +1 |
| Convenient        | <hr>        |    |
| Masochist         | Whenever your character takes [damage](#character-lifespan) they have advantage[^5] on their next [ability check](#ability-checks). | +2 |
| Beginners Luck    | Your character has advantage[^5] on any [ability check](#ability-checks) concerning something they have never done before. | +2 |
| Useful            | <hr>        |    |
| Special interest  | You can choose a **topic X** that your character is particularly interested in. They will always succeed in [ability checks](#ability-checks) concerning **topic X**. | +3 |
| Threatening glare | In a [fight](#fights) your characters party acts first. | +3 |
| Powerful          | <hr>        |    |
| Lucky Bastard     | You can reroll any [ability check](#ability-checks) once, if you want, but the last result counts. | +4 |
| Call bullshit     | Once per session your character can call bullshit on something, and the GM will have to change it. | +4 |
| Game changing     | <hr>        |    |
| Secret Plot/Plan  | Your character has an extra [revelation](#revelations) that they can reveal at any time.<br>(Not available for [crafters](#crafter)) | +5 |
...
| Risky               | <hr>        |    |
| On the run          | Your character is being persued by a **party X**[^6] of your choosing. At some point in the session (Whenever the GM thinks it appropriate) they will turn up and try to arrest/kill/etc. your character. | -3 |
| Target on their back| In a [fight](#fights), enemies will always attack your character first if possible. | -3 |
| Dangerous           | <hr>        |    |
| Bad Memory          | The GM can change **one thing** about your character at any time. | -4 |
| Clumsy              | If your character fails an [ability check](#ability-checks) you will have to roll another d10, and they will take 1 [damage](#character-lifespan) on a 1. | -4 |
| Suicidal            | <hr>        |    |
...
[Enemies](#npcs) act first, except if the GM decides otherwise[^8].\
